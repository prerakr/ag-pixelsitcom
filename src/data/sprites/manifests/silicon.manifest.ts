import { SpriteAtlasManifest } from '../../../types/sprite';

const ROW_H = Math.round(2048 / 6); // ~341px
const COL_W = 256;

function makeSiliconAnim(row: number) {
  const y = row * ROW_H;
  return {
    down: [
      { x: 0 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
      { x: 4 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
    ],
    up: [
      { x: 1 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
      { x: 5 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
    ],
    right: [
      { x: 2 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
      { x: 3 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
      { x: 6 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
      { x: 7 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
    ],
    left: [
      { x: 2 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92, flipX: true },
      { x: 3 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92, flipX: true },
      { x: 6 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92, flipX: true },
      { x: 7 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92, flipX: true },
    ],
    sitting: {
      down: { x: 0 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
      left: { x: 2 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92, flipX: true },
      right: { x: 2 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
      up: { x: 1 * COL_W, y, w: COL_W, h: ROW_H, anchorX: 0.5, anchorY: 0.92 },
    },
  };
}

export const SILICON_MANIFEST: SpriteAtlasManifest = {
  showId: 'silicon_valley',
  images: {
    silicon_characters: {
      url: '/sprites/silicon_characters.png',
      chromaKey: '#00ff00',
      tolerance: 55,
    },
  },
  characters: {
    richard: {
      characterId: 'richard',
      imageKey: 'silicon_characters',
      frameWidth: COL_W,
      frameHeight: ROW_H,
      scale: 0.175,
      enabled: true,
      animations: makeSiliconAnim(0),
    },
    gilfoyle: {
      characterId: 'gilfoyle',
      imageKey: 'silicon_characters',
      frameWidth: COL_W,
      frameHeight: ROW_H,
      scale: 0.18,
      enabled: true,
      animations: makeSiliconAnim(1),
    },
    dinesh: {
      characterId: 'dinesh',
      imageKey: 'silicon_characters',
      frameWidth: COL_W,
      frameHeight: ROW_H,
      scale: 0.175,
      enabled: true,
      animations: makeSiliconAnim(2),
    },
    erlich: {
      characterId: 'erlich',
      imageKey: 'silicon_characters',
      frameWidth: COL_W,
      frameHeight: ROW_H,
      scale: 0.19,
      enabled: true,
      animations: makeSiliconAnim(3),
    },
    jared: {
      characterId: 'jared',
      imageKey: 'silicon_characters',
      frameWidth: COL_W,
      frameHeight: ROW_H,
      scale: 0.185,
      enabled: true,
      animations: makeSiliconAnim(4),
    },
    jianyang: {
      characterId: 'jianyang',
      imageKey: 'silicon_characters',
      frameWidth: COL_W,
      frameHeight: ROW_H,
      scale: 0.17,
      enabled: true,
      animations: makeSiliconAnim(5),
    },
  },
  props: {},
  tiles: {},
  portraits: {},
};
